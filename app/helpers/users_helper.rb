  ...
  private
    ...
    def users_data(users)
      users.collect do |user|
        ...
        controlls << link_to_function("<i class='icon-eye-open'></i>".html_safe, "Application.load_content(Routes.user_path(#{user.id}))", :title => (t :user_view)) if can?(:read, Event)
        controlls << link_to_function("<i class='icon-pencil'></i>".html_safe, "Application.load_content(Routes.edit_user_path(#{user.id}))", :title => (t :user_edit)) if can?(:update, Event)
        controlls << link_to("<i class='icon-trash'></i>".html_safe, user_path(user),
                       :remote => true,
                       :method => :delete,
